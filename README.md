...
The installer creates a separate administrator token for update operations.
It is stored with mode `0600` in `~/.config/agents-server/env`; it is not the
normal API access token and should not be shared with ordinary clients. Add it
to AgentsDock Settings only on devices you trust to administer the host.

Managed update endpoints are authenticated twice: the normal bearer token is
still required by the API middleware, and the administrator token must also be
provided in `X-AgentsServer-Admin-Token`:
...
| `AGENTS_SERVER_ADMIN_TOKEN` | Separate administrator token for signed managed updates | generated by `install.sh` |
