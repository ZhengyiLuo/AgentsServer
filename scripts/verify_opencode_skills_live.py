...
provider, XDG, upload, and project state lives below one retained
temporary diagnostics directory. HOME is preserved; server skill discovery
uses an explicitly isolated home argument. It neither contacts nor restarts an
...
def main() -> None:
    ...
    parser.add_argument("--diagnostics-dir", type=Path, help="Existing development-volume directory for retained evidence")
    args = parser.parse_args()

    root = Path(tempfile.mkdtemp(prefix="opencode-skills-live-", dir=args.diagnostics_dir))
    ...
    inventory = agent_server.opencode_provider_skill_inventory

    def isolated_inventory(**kwargs):
        return inventory(**{**kwargs, "home": str(root / "home")})

    with patch.object(agent_server, "opencode_provider_skill_inventory", side_effect=isolated_inventory):
