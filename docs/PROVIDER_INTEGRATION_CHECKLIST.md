...
- [ ] Verify Delete includes releasing this chat's owned provider runtime/lock before reporting success, removes only AgentsDock-owned chat data, preserves native history, and allows native app/CLI resume; test cleanup failures and confirm other chats are unaffected for each provider/version.
