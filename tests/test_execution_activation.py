...
from tests import test_activation_transaction as fixtures
...
with ProcessLease(Path(sys.argv[2]), "worker") as lease:
    ...
    signal.pause()
'''
        child = subprocess.Popen([sys.executable, "-c", script, str(Path(__file__).resolve().parents[1]),
...
''')
        ...
        prefix = ('set -eu\nSTATE_ROOT='+shlex.quote(str(self.state))+'\nINSTALL_ROOT='+shlex.quote(str(self.item.root))+'\n'
            ...
            + 'execution_activation_command() { '+shlex.join([sys.executable, str(helper), str(Path(__file__).resolve().parents[1]), str(arguments)])+' "$@"; }\n')
