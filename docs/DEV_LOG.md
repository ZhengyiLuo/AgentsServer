...
## 2026-09-21 — Integrate completed server work into main

- Merge the completed server branch through `b715dcb9` with main `fa7ad114`,
  retaining custom endpoint effort controls and native retries, Claude Side chat,
  completed reasoning content, accurate failed scheduled turns, and faster chat
  synchronization alongside named server instances, OpenCode and runtime budgets.
- Keep recipient-bound `user_delegation` provenance from main and the mailbox
  lifecycle, bounded reads and original-scope execution guidance from the feature
  branch. Preserve authorization constraints without repeated approval for work
  already covered by the attested user instruction.
- Reconcile the test-directory move and CI sharding. All 237 modules import
  successfully and expose 4,861 test cases; this is discovery, not a full-suite
  execution claim. All 597 focused feature, instance, packaging, runtime,
  migration and WebSocket checks pass with isolated state and configuration.
- Start the actual recovered server on loopback with synthetic credentials and
  provider executables disabled. Verify authenticated health, create isolated
  Codex and Claude chats, read both timeline pages and list sessions. Shut down
  the test process cleanly. This smoke test does not run a model provider.
- Availability: source integration into main; no production server restart,
  deployment or new release artifact is part of this merge.
