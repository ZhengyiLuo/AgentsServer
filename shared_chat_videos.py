"""Read-only, signed references to videos already published in one chat.

No workspace paths, file discovery, registry writes, or video copies. Capabilities
...
def normalize_shared_chat_videos(value):
    ...
    for item in value:
        ...
        if (not isinstance(identity, str) or len(identity) > 1024 or VIDEO_ID.fullmatch(identity) is None
                ...
                or len(name.encode("utf-8")) > 1024 or not isinstance(item["content_type"], str) or item["content_type"] not in VIDEO_TYPES
                or type(item["size"]) is not int or not 0 < item["size"] <= (1 << 53) - 1):
            raise ValueError("Invalid shared video")
...
def _key(secret):
    ...
    return hmac.new(secret.encode("utf-8"), b"agentsdock-chat-video-v1", hashlib.sha256).digest()
...
def _open_registered(root, session_id, file_id, *, legacy_owner=False):
    ...
    try:
        ...
        if (not isinstance(name, str) or not 1 <= len(name) <= 255 or name in {".", "..", "meta.json"}
            ...
            raise SharedVideoUnavailable()
        guessed = EXTENSIONS.get(Path(name).suffix.lower())
        mime = str(meta.get("content_type") or "").split(";", 1)[0].strip().lower()
        if mime in {"", "application/octet-stream", "binary/octet-stream"}:
            mime = guessed
        if mime not in VIDEO_TYPES or mime != guessed:
            raise SharedVideoUnavailable()
        video_fd = os.open(name, os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=directory_fd)
        info = os.fstat(video_fd)
        _regular(info, owner)
        if type(meta.get("size")) is not int or meta["size"] != info.st_size or not 0 < info.st_size <= (1 << 53) - 1:
            raise SharedVideoUnavailable()
...
def shared_chat_video_descriptor(root, secret, session_id, file_id, *, legacy_owner=False):
    key = _key(secret)
    opened = _open_registered(root, session_id, file_id, legacy_owner=legacy_owner)
    try:
        payload = json.dumps([1, session_id, file_id, opened["revision"]], separators=(",", ":")).encode("ascii")
        encoded = base64.urlsafe_b64encode(payload).rstrip(b"=").decode("ascii")
        identity = "video_" + encoded + "." + hmac.new(key, encoded.encode("ascii"), hashlib.sha256).hexdigest()
        return normalize_shared_chat_videos([{key: opened[key] for key in ("filename", "content_type", "size")} | {"id": identity}])[0]
    finally:
...
def open_shared_chat_video(root, secret, session_id, identity):
    ...
    try:
        if not isinstance(identity, str) or len(identity) > 1024 or VIDEO_ID.fullmatch(identity) is None:
            raise SharedVideoUnavailable()
        encoded, signature = identity[len("video_"):].split(".")
        expected = hmac.new(_key(secret), encoded.encode("ascii"), hashlib.sha256).hexdigest()
        ...
        if (not isinstance(payload, list) or len(payload) != 4 or type(payload[0]) is not int or payload[0] != 1
            ...
            raise SharedVideoUnavailable()
        opened = _open_registered(root, session_id, payload[2], legacy_owner=True)
