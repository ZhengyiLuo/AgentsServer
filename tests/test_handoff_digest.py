...
class DigestDeliveryTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_legacy_queued_digest_recovery_pins_its_persisted_backend(self) -> None:
        for backend in (agent_server.BACKEND_CODEX, agent_server.BACKEND_CLAUDE):
            with self.subTest(backend=backend):
                target = {**agent_server.STORE.sessions["target-1"], "backend": backend}
                event = {
                    "type": "turn_queued", "queued_id": "queued-legacy",
                    "purpose": "handoff_digest_delivery", "digest_job_id": "digest-1",
                    "source_session_id": "source-1", "target_session_id": "target-1",
                    "backend": backend, "prompt": "legacy digest",
                }
                recovered = agent_server.queued_turn_from_event(event, target, 1)
                req = agent_server.TurnRequest(**{
                    key: recovered[key] for key in (
                        "prompt", "purpose", "digest_job_id", "source_session_id",
                        "target_session_id", "backend", "client_capabilities",
                    )
                })
                agent_server.validate_handoff_digest_delivery_admission(
                    "target-1", req, target, provider_context_mode="chat",
                )
                self.assertEqual(req.client_capabilities,
                    agent_server.handoff_digest_target_client_capabilities(target))
                changed = {**target, "backend": "opencode"}
                with self.assertRaises(agent_server.HTTPException):
                    agent_server.validate_handoff_digest_delivery_admission(
                        "target-1", req, changed, provider_context_mode="chat",
                    )

    def test_unpinned_new_digest_request_is_not_legacy_migrated(self) -> None:
        req = agent_server.TurnRequest(
            prompt="digest", purpose="handoff_digest_delivery", digest_job_id="digest-1",
            source_session_id="source-1", target_session_id="target-1",
        )
        with self.assertRaises(agent_server.HTTPException):
            agent_server.validate_handoff_digest_delivery_admission(
                "target-1", req, agent_server.STORE.sessions["target-1"],
                provider_context_mode="chat",
            )

    def test_legacy_digest_without_persisted_backend_stays_unpinned(self) -> None:
        recovered = agent_server.queued_turn_from_event({
            "type": "turn_queued", "queued_id": "queued-legacy",
            "purpose": "handoff_digest_delivery", "digest_job_id": "digest-1",
            "source_session_id": "source-1", "target_session_id": "target-1",
            "prompt": "legacy digest",
        }, agent_server.STORE.sessions["target-1"], 1)
        self.assertEqual(recovered["client_capabilities"], [])

    def test_legacy_digest_pin_survives_later_queue_edits(self) -> None:
        target = agent_server.STORE.sessions["target-1"]
        queued = {
            "type": "turn_queued", "queued_id": "queued-legacy", "seq": 1,
            "purpose": "handoff_digest_delivery", "digest_job_id": "digest-1",
            "source_session_id": "source-1", "target_session_id": "target-1",
            "backend": "claude", "prompt": "legacy digest", "client_capabilities": [],
        }
        edited = {
            "type": "turn_queue_updated", "queued_id": "queued-legacy", "seq": 2,
            "prompt": "edited digest", "client_capabilities": [],
        }
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "events.jsonl"
            path.write_text("".join(json.dumps(event) + "\n" for event in (queued, edited)))
            with patch.object(agent_server, "events_path", return_value=path):
                recovered = agent_server.scan_queued_turns_from_events([("target-1", target)])
        row = recovered["target-1"][0]
        self.assertEqual(row["prompt"], "edited digest")
        self.assertEqual(row["client_capabilities"],
            agent_server.handoff_digest_target_client_capabilities(target))

    def test_current_digest_pin_is_not_legacy_migrated_after_queue_edit(self) -> None:
        target = agent_server.STORE.sessions["target-1"]
        queued = {
            "type": "turn_queued", "queued_id": "queued-current", "seq": 1,
            "purpose": "handoff_digest_delivery", "digest_job_id": "digest-1",
            "source_session_id": "source-1", "target_session_id": "target-1",
            "backend": "claude", "prompt": "current digest",
            "client_capabilities": agent_server.handoff_digest_target_client_capabilities(target),
        }
        edited = {
            "type": "turn_queue_updated", "queued_id": "queued-current", "seq": 2,
            "client_capabilities": [],
        }
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "events.jsonl"
            ...
            with patch.object(agent_server, "events_path", return_value=path):
                recovered = agent_server.scan_queued_turns_from_events([("target-1", target)])
        self.assertEqual(recovered["target-1"][0]["client_capabilities"], [])
