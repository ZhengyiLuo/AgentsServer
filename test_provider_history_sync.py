...
class UnsyncedHistoryItemsTests(unittest.TestCase):
    ...
    def select(self, events, items):
        with tempfile.TemporaryDirectory() as directory:
            ...
            event_file.write_text(
                "".join(
                    json.dumps({"seq": index, "session_id": "chat-x", **event})
                    + "\n"
    ...
    def test_compacted_scheduled_result_anchors_first_sync(self) -> None:
        # Scheduled job history may be compacted down to its canonical final
        # result. Its provider copy also has a sanitized prompt, so the final
        # result is the only exact recent anchor. Without it, opening the chat
        # re-imports the whole block of cron runs as ordinary messages.
        events = [
