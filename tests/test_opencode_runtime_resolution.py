...
class OpenCodeAdmissionResolutionTests(unittest.IsolatedAsyncioTestCase):
    async def test_cached_ready_runtime_rechecks_exact_version_without_auth_or_models(self):
        ready = {"backend": "opencode", "status": "ready", "_executable": "/test/opencode"}

        def command(argv):
            help_text = "opencode run " + " ".join(agent_server.OPENCODE_REQUIRED_CLI_FLAGS)
            return subprocess.CompletedProcess(argv, 0, help_text if argv[-1] == "--help" else "1.18.30", "")

        with patch.object(agent_server, "runtime_diagnostic", return_value=ready), \
             patch.object(agent_server, "runtime_command", side_effect=command) as probe, \
             patch.object(agent_server, "store_runtime_diagnostic"):
            with self.assertRaises(agent_server.HTTPException) as raised:
                await agent_server.ensure_runtime_available("opencode")
        self.assertEqual(raised.exception.status_code, 503)
        self.assertIn("1.18.29", raised.exception.detail["message"])
        self.assertEqual([call.args[0] for call in probe.call_args_list], [
            ["/test/opencode", "run", "--help"], ["/test/opencode", "--version"],
        ])
